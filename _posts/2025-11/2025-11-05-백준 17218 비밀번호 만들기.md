...
두 문자열에서 공통으로 존재하는 가장 긴 부분 문자열을 구하는 문제로 LCS를 구해야 하는 문제다. 다만 LCS의 길이가 아니라 실제 부분 문자열을 구해야 한다는 점에서 다이나믹 프로그래밍을 활용해 LCS를 구한 후 역추적을 통해 실제 문자열 정보를 얻어야 한다.([LCS]({% post_url 2025-11/2025-11-16-LCS (Longest Common Subsequence) %}))
