...
`l`부터 `r`까지의 정수를 하나씩 확인하며 각 자리수가 전부 0 또는 5로만 이루어졌는지 판별한다. 판별 함수는 숫자를 10으로 나눈 나머지를 확인하는 과정을 자릿수가 다 없어질 때까지 반복하다가, 0도 5도 아닌 자리가 하나라도 나오면 그 즉시 `false`를 반환하고 그렇지 않으면 `true`를 반환한다. 조건을 만족하는 수를 오름차순으로 모으고, 하나도 없으면 -1을 담은 배열을 대신 반환한다.
