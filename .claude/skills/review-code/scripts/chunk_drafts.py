...
**청크 크기 기준(드래프트 최대 4개/총 줄수 약 1200줄)의 설계 이유는
review-code/SKILL.md, review-post/SKILL.md에 문서화돼있음 — 여기서 다시 설명
안 함. `--mode code`/`--mode post`는 지금은 동작이 동일(둘 다 필터 없이 전체
대상) — review-post가 예전엔 '2. 접근' 비어있는 드래프트를 스킵했었는데, 이제
비어있으면 review-post가 직접 채우는 게 정상 동작이라 필터 자체를 없앰. 모드
구분은 향후 다시 갈릴 가능성을 열어두려고 인터페이스만 유지.**
