...
모스부호를 해독하는 문제로 모스부호와 알파벳 소문자를 1대1로 대응시키는 표를 만들고, 공백으로 구분된 모스부호 문자열 `letter`를 토큰 단위로 분리해 각각을 대응되는 문자로 치환하면 되는 문제다. 모스부호 사이 공백은 항상 하나이고 해독 불가능한 입력은 주어지지 않으므로, 공백 기준으로 자른 각 토큰을 표에서 조회해 이어 붙이기만 하면 된다.
