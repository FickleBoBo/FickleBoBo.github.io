...
세그먼트 트리를 활용해 해결할 수 있는 흥미로운 문제 중 하나는 K번째 수를 찾는 문제다. 구간 합을 구하는 [세그먼트 트리](/posts/segment-tree) 와 카운팅 배열, 이분 탐색의 아이디어를 종합하면 세그먼트 트리에서 K번째로 작은 수를 $O(\log{N})$ 의 시간복잡도로 찾을 수 있다.
