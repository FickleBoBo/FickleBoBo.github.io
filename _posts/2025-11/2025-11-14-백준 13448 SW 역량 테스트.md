...
따라서 $\frac{P}{R}$ 가 큰 문제를 먼저 풀도록 정렬을 수행한 후 0／1 배낭 문제를 적용하면 된다.
...
$\dfrac{P}{R}$ 가 큰 수가 앞에 오도록 정렬을 해야한다. 분수로 다루기 보다는 정수가 안전하니 $P_A R_B > P_B R_A$ 로 다루었다. 정렬은 리턴 값이 음수가 나와야 앞에 위치하므로 $P_B R_A - P_A R_B$ 를 리턴하게 `Comparator`를 작성했다. 점수를 계산하는 $M - t * P$ 는 `t`과 `P`가 최대 `100,000`이어서 바로 곱하면 오버플로우가 발생할 수 있어 `P`를 `long` 타입으로 처리하였다.
