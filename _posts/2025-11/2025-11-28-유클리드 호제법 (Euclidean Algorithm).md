...
유클리드 호제법은 두 자연수 또는 다항식의 최대공약수를 구하는 알고리즘의 하나이다. 호제법이란 말은 두 수가 서로 상대방 수를 나누어서 결국 원하는 수를 얻는다는 뜻이다.
...
$$
\begin{aligned}
78696 &= 19332 \times 4 + 1368 \\[4pt]
19332 &= 1368 \times 14 + 180 \\[4pt]
1368  &= 180 \times 7 + 108 \\[4pt]
180   &= 108 \times 1 + 72 \\[4pt]
108   &= 72 \times 1 + 36 \\[4pt]
72    &= 36 \times 2 + 0
\end{aligned}
$$

$72$ 을 $36$ 으로 나눈 나머지가 $0$ 이 됐다. 따라서, $78696$ 과 $19332$ 의 최대공약수는 $36$ 이다.

TMI로 최대공약수는 GCD (Greatest Common Divisor), 최소공배수는 LCM(Least Common Multiple)으로 불려서 PS에서 함수나 메서드명도 여기에 맞춰준다.
...
$$
\begin{aligned}
\phantom{\Rightarrow\quad} & a = bq + r \qquad (0 \le r < b) \\[8pt]
...
재귀를 활용하면 유클리드 호제법 코드를 깔끔하게 작성할 수 있다. 두 수 $a$, $b$ 를 파라미터로 받아서 $b = 0$ 이면 $a$ 를 반환하고 아니면 $b$, $r$ 을 파라미터로 받는 재귀 함수를 반환하면 된다. $a$ 가 $b$ 로 나누어 떨어지는 경우 $b$ 가 최대공약수가 되며 $r$ 이 $0$ 이 돼서 다음 재귀 때 최대공약수를 그대로 반환하며 종료할 수 있다.

### 유클리드 호제법 [Java]

$a < b$ 인 경우 다음 재귀 때 $a$, $b$ 의 순서만 바뀐채로 재귀가 돌아가서 두 수의 크기를 고려하지 않아도 동작한다.
