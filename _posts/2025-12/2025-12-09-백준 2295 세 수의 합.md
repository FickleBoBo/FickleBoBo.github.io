...
자연수들로 이루어진 집합 $U$ 에서 고른 세 수의 합이 $U$ 에 포함되는 가장 큰 수를 찾는 문제다. $N$ 이 최대 $1,000$ 이어서 3중 반복문을 통한 풀이는 시간 초과가 발생하게 되는데 이를 2중 반복문 두 개로 쪼개서 해결하는 중간에서 만나기 알고리즘을 적용해야 한다. 핵심은 $a + b + c = d$ 를 만족하는 $d$ 를 찾을 때, $a + b = d - c$ 로 찾는 것이다. 이분 탐색을 활용한 방법과 집합을 활용한 방법 두 가지로 해결했다.
...
| 언어   | 시간  | 메모리  | 코드 길이 |
| ------ | ----- | ------- | --------- |
| C++ 17 | 24 ms | 5228 KB | 807 B     |

### 4. 해시를 사용한 집합과 맵 [C++]

| 언어   | 시간   | 메모리   | 코드 길이 |
| ------ | ------ | -------- | --------- |
| C++ 17 | 112 ms | 23312 KB | 720 B     |
