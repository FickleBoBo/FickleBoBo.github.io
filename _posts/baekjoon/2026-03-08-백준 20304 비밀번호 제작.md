...
핵심 로직은 특정 수와 안전 거리가 1인 수들을 찾는 것으로 특정 수를 `cur`이라 했을 때, `cur ^ (1 << i)`로 `i`를 `0`부터 증가시키며 안전 거리가 1인 다음 수들을 모두 뽑아낼 수 있다. 비밀번호의 최댓값이 1,000,000이어서 `1 << 19`까지만 비트를 뒤집어보면 된다.
