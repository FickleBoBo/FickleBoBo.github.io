...
toc: true
math: true
image: /assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/thumbnail.drawio.svg
---
...
![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo01.drawio.svg>)

먼저 주어진 원소들을 두 부분으로 분할한다. 분할은 중간 위치를 기준으로 분할하며 주어진 원소가 하나일 때까지 반복한다. 현재 1번부터 4번까지가 분할한 앞 부분이 된다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo02.drawio.svg>)

아직 분할할 수 있으므로 두 부분으로 분할한다. 현재 1번부터 2번까지가 분할한 앞 부분이 된다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo03.drawio.svg>)

아직 분할할 수 있으므로 두 부분으로 분할한다. 현재 1번부터 1번까지가 분할한 앞 부분이 된다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo04.drawio.svg>)

더 이상 분할할 수 없으므로 이전으로 돌아가 이전에 분할했던 뒷 부분에 대해 똑같이 시도한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo05.drawio.svg>)

1번부터 2번 구간에 대한 분할이 끝났으므로 병합한다. 병합은 2칸짜리 임시 공간을 만들고 병합할 앞 부분과 뒷 부분의 첫 번째 원소부터 작은 것들을 순서대로 넣어준다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo06.drawio.svg>)

1번부터 4번 구간에 대한 앞 부분의 병합이 완료됐다. 이제 뒷 부분에 대해 분할을 시작한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo07.drawio.svg>)

더 이상 분할할 수 없을 때까지 분할한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo08.drawio.svg>)

더 이상 분할할 수 없을 때까지 분할한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo09.drawio.svg>)

3번부터 4번 구간에 대한 분할이 끝났으므로 병합한다. 병합은 2칸짜리 임시 공간을 만들고 병합할 앞 부분과 뒷 부분의 첫 번째 원소부터 작은 것들을 순서대로 넣어준다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo10.drawio.svg>)

1번부터 4번 구간에 대해 앞 부분의 병합과 뒷 부분의 병합이 각각 완료됐다. 이제 앞 부분과 뒷 부분도 병합해준다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo11.drawio.svg>)

1번부터 8번 구간에 대해 앞 부분의 병합이 완료됐다. 이제 5번부터 8번 구간에 대해 똑같이 진행한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo12.drawio.svg>)

5번부터 6번 구간으로 분할해준다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo13.drawio.svg>)

5번부터 5번 구간으로 분할해준다. 더 이상 분할할 수 없으므로 종료한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo14.drawio.svg>)

6번부터 6번 구간으로 분할해준다. 더 이상 분할할 수 없으므로 종료한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo15.drawio.svg>)

5번부터 6번 구간에 대한 분할이 끝났으므로 병합해준다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo16.drawio.svg>)

5번부터 8번 구간의 뒷 부분에 대해 분할을 시작한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo17.drawio.svg>)

7번부터 7번 구간으로 분할해준다. 더 이상 분할할 수 없으므로 종료한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo18.drawio.svg>)

8번부터 8번 구간으로 분할해준다. 더 이상 분할할 수 없으므로 종료한다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo19.drawio.svg>)

7번부터 8번 구간에 대한 분할이 끝났으므로 병합해준다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo20.drawio.svg>)

5번부터 8번 구간에 대한 앞 부분 병합과 뒷 부분 병합이 각각 완료됐다. 이제 앞 부분과 뒷 부분을 병합해준다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo21.drawio.svg>)
...
![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo22.drawio.svg>)

전체 구간에 대한 병합을 완료했으므로 병합 정렬을 종료한다. 이렇게 병합 정렬을 통해 주어진 수들을 오름차순으로 정렬하는데 성공했다.

![](</assets/posts/2025-11/병합%20정렬%20(Merge%20Sort)/photo23.drawio.svg>)
