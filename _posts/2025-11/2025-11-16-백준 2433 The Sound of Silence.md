...
[덱을 이용한 구간 최대/최소 트릭]({% post_url 2025-11/2025-11-16-덱을 이용한 구간 최대／최소 트릭 (Sliding Window Max／Min with a Monotonic Deque) %}) 을 활용하면 해결할 수 있는 문제로 구간의 최솟값과 최댓값이 모두 필요한데 이를 각각 단조 증가 덱, 단조 감소 덱으로 관리하며 최댓값과 최솟값의 차를 `c`와 비교하는 방식으로 해결했다.
