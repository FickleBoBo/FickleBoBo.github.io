---
title: "[백준] 1064번 - 평행사변형 [Java][C++]"
date: 2025-12-07
categories: [PS, BaekJoon]
tags: [수학, 기하학, 피타고라스 정리, CCW]
toc: true
math: true
---
...
평행사변형을 만들 수 없는 경우는 세 점이 일직선 상에 위치하는 경우로 세 점의 좌표가 $(x_A, y_A)$, $(x_B, y_B)$, $(x_C, y_C)$ 일 경우 $\dfrac{y_B-y_A}{x_B-x_A} = \dfrac{y_C-y_A}{x_C-x_A}$ 로 기울기를 비교하면 일직선 상에 위치하는지 알 수 있다. 이때 $x$ 좌표가 일치할 수도 있고 나눗셈의 경우 나머지가 버림처리 되며 실수 타입은 부동소수점 오차가 생길 수 있기 때문에 $(y_B-y_A)\cdot(x_C-x_A) = (y_C-y_A)\cdot(x_B-x_A)$ 로 계산했다. 또는 CCW 알고리즘을 활용해 세 점이 일직선 상에 위치하는지 판단해도 된다.
...
![](/assets/posts/2025-12/백준%201064%20평행사변형/photo01.drawio.svg)
![](/assets/posts/2025-12/백준%201064%20평행사변형/photo02.drawio.svg)
![](/assets/posts/2025-12/백준%201064%20평행사변형/photo03.drawio.svg)
...
| 언어   | 시간 | 메모리  | 코드 길이 |
| ------ | ---- | ------- | --------- |
| C++ 17 | 0 ms | 2036 KB | 583 B     |

### 4. CCW [C++]

| 언어   | 시간 | 메모리  | 코드 길이 |
| ------ | ---- | ------- | --------- |
| C++ 17 | 0 ms | 2036 KB | 731 B     |
